...
class WearSystem(System):
    ...
    def doWear(self, intentionComponent):
        inventoryComponent = self.entityStorage.getEntityComponent(
            intentionComponent.entityId, InventoryComponent)
        if intention.ACTION_INVENTORY_LEFT in intentionComponent.actions:
            if inventoryComponent.pointer > 0:
                inventoryComponent.pointer -= 1
        if intention.ACTION_INVENTORY_RIGHT in intentionComponent.actions:
            if inventoryComponent.pointer < inventoryComponent.capacity - 1:
                inventoryComponent.pointer += 1
        if intention.ACTION_INVENTORY_EQUIP in intentionComponent.actions:
            statsComponent = self.entityStorage.getEntityComponent(
                intentionComponent.entityId, StatsComponent)
